...
Figure 1 (notes/hmi-mockups/remarkable/fig_console_sketch-8.png):
notional console, banner with acknowledge, resolution options, chart,
source roster, event log.
...
deterministic path will present the alert and initial informational
recommendation less than 100 milliseconds after input receipt at the
system boundary, measured to the first displayed frame containing both
the alert and initial recommendation, with security controls enabled.
Supporting explanation may follow through local drill-down. The design
will maintain this initial response even when the local model is
unavailable. No ship controls will be actuated. The work develops no
...
analyses and a transition plan. Human workload, comprehension and
decision-time improvements remain outcomes to establish through an
appropriately authorized later evaluation.
...
operator drill-down, so retrieval never threatens the proposed 100 ms
gate. This is a target to be measured, not a result.
...
packaging, not accreditation. Whether an Iron Bank image is required
for the Phase I demonstration remains an open Government question; the
secured-runtime gate applies regardless.

1.1 PHASE I TECHNICAL OBJECTIVES

The six-month base will answer three questions: can the interface
preserve and explain multi-source evidence; can it distinguish
supported recommendations from unresolved alternatives; and can it do
so within a secured, air-gapped execution envelope with a proposed
...
secured runtime are fixed constraints. We propose a tighter <100 ms
response gate; it is an offeror target, not a Government-stated
threshold. The control checklist, case counts and other numerical
gates are proposed engineering targets. Before tuning, freeze the
...
<100 ms response (proposed)
In the hardened configuration, measure input receipt at the system
boundary to the first displayed frame containing both alert and
initial recommendation, using a monotonic clock. Test 3/8-source x
1/10-Hz corners, intermediate/mixed rates, and local-model failure.
Any missing response or measured interval >=100 ms fails the proposed
gate. Report all events, maximum, distribution and drops.
...
Information access
On 12 frozen tasks, required evidence must be reachable and complete,
with no critical legibility/priority defect under a predefined design
checklist. Compare views/interactions with the fragmented surrogate
and an explanation-disabled unified view. Counts are reported as
workflow proxies, without a promised percentage improvement.

The Government requirement is under one second and will be reported
separately. Any event at or above 100 ms fails the proposed gate. A
failed proposed gate remains a reported feasibility limitation, not a
requirement waiver. The evaluation protocol specifies event counts,
timing instrumentation, case selection, security tests and comparison
controls. The final report will include results, unresolved
integration risks and the Phase II plan.

Participant research is not included. The CUI handling scope and any
evaluation activity requiring a determination must be resolved with
the Government; synthetic data do not establish public releasability.
Iron Bank timing remains an open implementation question, without
relaxing the secured-runtime requirement. [1]
...
Personnel before submission. Evaluation will use synthetic inputs and
automated replay; the base effort does not require human-subject
research, shipboard access, or government-furnished equipment.
...
prerequisite-bound recommendations. Report INDETERMINATE when
...
Deliver: integrated feasibility prototype and evidence-linked traces.
...
unauthorized access, package tampering, and audit failure. Test the
proposed <100 ms
input-receipt-to-first-displayed-alert-and-recommendation target with
security enabled; any measured event at or above 100 ms fails that
target. Report the topic's <1 s requirement separately. A failed
proposed gate is a reported feasibility limitation, not a waiver.
...
checks, repeated replays, and information-access checks defined in the
evaluation protocol. Run four 30-minute source/rate corners, a
...
Include queueing and actual frame presentation; uncertainty
overlapping 100 ms does not establish a pass. Compare against a
...
training dataset, rule provenance (rule to guidance clause), and
regression results.
...
O1. Resolve feasibility findings | Option Months 1-2
...
O2. Prepare integration and deployment | Option Months 2-4
...
O3. Establish Phase II readiness | Option Months 4-6
...
recommendation (offeror-proposed under 100 ms; Government requirement
under one second). A notional console layout (source health,
...
providers. Earlier: Department of Defense computer scientist
(2002-2003, Ada-to-C++ conversion); U.S. Navy EM4, USS Kalamazoo
(AOR-6), 1993-1996. University of Central Oklahoma, computer science,
...
product-owner-facing roles. His prior Department of Defense
programming work and Navy sea service give shipboard context for the
interface work; they are not presented as GPNTS or APNT-specific
expertise. [base hours / option hours to confirm]
...
engineering targets:
- Month 6 (Proposed): Complete Phase I feasibility report; measure the
proposed <100 ms gate from input receipt at the system boundary to the
first displayed frame containing both the alert and initial
recommendation, with security enabled. Any event >=100 ms fails the
proposed gate. Report the Government's <1 s requirement separately; a
failed proposed gate is a reported feasibility limitation, not a
waiver. Verify air-gapped operation.
